...
pub inline fn send(self: UdpClient, data: []const u8) ClientSendError!usize {
    if (!self.udp_core.bound.load(.acquire))
        return ClientSendError.NotConnected;

    return posix.write(self.udp_core.socket, data);
}

/// Blocks until all data has been sent through the connected socket.
///
/// Returns `NotConnected` if the client is not connected.
pub fn sendAll(self: UdpClient, data: []const u8) ClientSendError!void {
    if (!self.udp_core.bound.load(.acquire))
        return ClientSendError.NotConnected;

    var total_sent: usize = 0;
    while (total_sent < data.len) {
        total_sent += posix.write(self.udp_core.socket, data[total_sent..]) catch |err| switch (err) {
